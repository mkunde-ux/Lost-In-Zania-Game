...
public class EuodiaAI : Guard
{
    //Check SecurityAI script for comments
    ...
    public EuodiaConDialogue dialogueScript;
    private bool dialogueCompleted = false;
    private HandReach handReach;

    [Header("References")]
    private Transform _player;
    public List<Transform> playersInMemory = new List<Transform>();
    ...
    public EuodiaMovement securityMovement;
    ...
    void Start()
    {
        securityMovement = GetComponent<EuodiaMovement>();
    ...
    private IEnumerator MoveGuardToPlayer()
    {
        if (player != null)
        {
            ...
            {
                Debug.LogError("Dialogue script not assigned in EuodiaAI!");
            }
        }
    }
    ...
    private void HandleChasing()
    {
        ...
        if (fieldOfView != null && (fieldOfView.canSeePlayer || playersInMemory.Contains(player)))
        {
            ...
            chaseTimerCoroutine = StartCoroutine(ResetChaseTimer());
        }
        else
        {
            if (chaseTimer <= 0)
