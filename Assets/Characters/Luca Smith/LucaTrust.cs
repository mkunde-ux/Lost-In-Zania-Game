...
public class LucaTrust : MonoBehaviour
{
    ...
    public LucaTrustGuage trustBar; 
    public LucaDialogue dialogueTree; 
    ...
    private void Start()
    {
        ...
        {
            Debug.LogError("Trust bar reference not assigned in Luca!");
        }
    }
    ...
    public void CallSecurityAndCloseDialogue()
    {
        ...
        {
            dialogueTree.dialogueText.text = "=Luca: Okay, that’s it! You think you're funny? I’m calling Security!!";

            StartCoroutine(DelayBeforeCallingGuard(2f));          
        }
        else
        {
            Debug.LogError("DialogueTree reference is missing in Luca!");
        }
    }
    ...
    public void CallSecurity()
    {
        if (securityGuard != null)
        {
            ...
            if (playerTransform != null)
            {
                ...
                Debug.Log("Luca has alerted the security guard!");
            }
            else
            {
        ...
        {
            Debug.LogWarning("Security guard reference not set for Luca!");
        }
    }
    ...
    public void CheckMizikiLevelAtEnd()
    {
        if (currentTrust <= maxTrust * (trustThresholdToAlertGuard / 100f))
        {
            Debug.Log("Luca's trust is too low. She's not happy!");
        }
        else
        {
            Debug.Log("Luca's trust is acceptable. Dialogue ended peacefully.");
        }
    }
}